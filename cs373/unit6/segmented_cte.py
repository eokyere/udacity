...
def run(grid, goal, spath, params, printflag = False, speed = 0.1, timeout = 1000):
    ...
    while not myrobot.check_goal(goal) and N < timeout:
        ...
        # Given two points P1 [x1 y1] and P2 [x2 y2] as the current path segment
        # dx = x2 - x1, dy = y2 - y1
        
        # Assuming the robot is estimated at point P [x y]
        # Let R represent the vector P1 P, such that Rx = x - x1, Ry = y-y1
        
        # Let u be how far robot has progressed over the segment P1 P2,
        # u = Rx.dx + Ry.dy / (dx^2 + dy^2)
        # if u > 1, robot has left that segment
